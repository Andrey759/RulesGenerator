...
public enum FlatFields {
    ...
    intercom("Домофон"),
    floor("Этаж"),
    numberOfLevelsInFlat("Количество этажей квартиры"),
    door("Входная дверь"),
    ...
    public static FlatFields findByText(String text) {
        for(FlatFields val : values()) {
            if(val.toString().equalsIgnoreCase(text)) {
                return val;
            }
        }
        throw new RuntimeException("Cant't find field '" + text + "' in FlatFields");
    }
